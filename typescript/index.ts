...
function saveSettings(): void {
  if (!GLOBAL_STATE.settings) return;
  try {
    fs.writeFileSync(PATHS.settings, JSON.stringify(GLOBAL_STATE.settings, null, 2));
  } catch (e: any) { logger.log(`Error saving settings: ${e.message}`, true); }
}
...
async function main(): Promise<void> {
  ...
  if (!GLOBAL_STATE.settings.overlay) GLOBAL_STATE.settings.overlay = { enabled: false, port: 3000, layout: "full", visualizer: {} as any };
  ...
  const devices = await visualizer.listDevices();

  const buildTrayMenu = () => {
    const s = GLOBAL_STATE.settings!;
    const currentDevice = s.overlay.visualizer.audioDevice || "default";
    const currentLayout = s.overlay.layout || "full";
    ...
    return {
      ...
      items: [
        ...
        { title: "Separator", enabled: false, __id: 100 },
        {
          title: "Audio Device",
          enabled: true,
          __id: 10,
        ...
        },
        {
          ...
          __id: 20,
          items: [
            { title: "Full", enabled: true, checked: currentLayout === "full", __id: 2001 },
            { title: "Compact Bar", enabled: true, checked: currentLayout === "compact-bar", __id: 2002 },
            { title: "Minimalist", enabled: true, checked: currentLayout === "compact-minimal", __id: 2003 },
          ],
        },
        { title: "Separator", enabled: false, __id: 101 },
  ...
  const tray = new TrayController(buildTrayMenu());
  
  // Custom tray click handler
  const handleTrayClick = (action: any) => {
    if (!GLOBAL_STATE.settings) return;
    
    // Audio Device selection
    if (action.item.__id >= 1000 && action.item.__id < 2000) {
      const newDevice = action.item.title;
      if (GLOBAL_STATE.settings.overlay.visualizer.audioDevice !== newDevice) {
        GLOBAL_STATE.settings.overlay.visualizer.audioDevice = newDevice;
        saveSettings();
        logger.log(`Audio Device changed to: ${newDevice}`, true);
        visualizer.kill(); // Will be restarted by startCapture loop or we can manually restart
        // If it's already running, it will restart automatically due to the close event in visualizer.ts
      }
    }
    
    // Layout selection
    if (action.item.__id >= 2001 && action.item.__id <= 2003) {
      const layouts: Record<number, any> = { 2001: "full", 2002: "compact-bar", 2003: "compact-minimal" };
      const newLayout = layouts[action.item.__id];
      if (GLOBAL_STATE.settings.overlay.layout !== newLayout) {
        GLOBAL_STATE.settings.overlay.layout = newLayout;
        saveSettings();
        logger.log(`Layout changed to: ${newLayout}`, true);
        // Overlay clients will refresh or we can broadcast a reload if needed
        serverMod.broadcast({ type: "layout_update", layout: newLayout });
      }
    }

    // Refresh tray menu to update checkmarks
    tray.sendAction({ type: "update-menu", ...buildTrayMenu() });
  };

  // We need to modify TrayController to accept this handler
  await (tray as any).init(cleanup, handleTrayClick);
